...
class PagesController extends Controller
{
    ...
    {
        $articles = Articles::latest('created_at')->paginate(4);
        // return View::make('pages.report', array('articles' => $articles));
        return view('pages.report', compact('articles'));
    }
    ...
    public function page_postArticle(Request $request)
    {
            $article = new Articles;
            $article->uploadArticle($request);
    }
    ...
    {
        $user = new User();
        $typeAccount = $user->getTypeAccount($request);
        $authen[0] = AUTHEN_FAILED;
        if ($typeAccount != null) {
            $authen[0] = AUTHEN_SUCCESS;
            $authen[1] = $request['username'];
    ...
    {
        $fileUrl= DB::table('articles')
        ->where('id', $id)->value('fileUrl');
        return view('pages.viewpost')->with('content', $fileUrl);
