    ...
    public function getIntro(){
     ...
     if($file = $request->filename{
        ...
        $input['your_file'] = $name;
				...
				$name = array_get($input,'title');
				$intro = array_get($input,'post-text');
				// SET UPLOAD PATH
				$destinationPath = 'docfile';
				$destinationPath2 = 'imagedocfile';
				...
				$upload_success_img = $img->move($destinationPath2,$imgName);
				if ($upload_success_file && $upload_success_img) {
          // $fileUrl = str_replace(" ", "%20", url('/docfile/')."/".$request->filename."");
					$fileUrl = "docfile"."//".$request->filename;
					$imgUrl =  "imagedocfile"."//".$request->imgname."";
				//insert to database
					if(!isset($_SESSION))
					{
						session_start();
					}
					if(isset($_SESSION['status_name']))// insert to database
					{
...
// 		}
    public function uploadFileCustom(Request $request){
		...
		public function getRecentlyArticlesByCategory($category){
			return DB::table('articles')
				->where('category',$category)
				->paginate(10);
		}
		public function getRecentlyArticleStudyPage($category){
			return DB::table('articles')
				->where('category', $category)
